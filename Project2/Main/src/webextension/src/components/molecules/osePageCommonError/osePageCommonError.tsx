...
import SomethingWentWrong from "../../../images/somethingwent wrong.svg";
...
const PageCommonError: React.FC<IEmptyContainerImageProps> = ({
  ...
  const handleMCTRedirect = () => {
    if (parentContext) {
      const route = "4afa8f62-a142-11ed-a8fc-0242ac120004"; //TODO: Read it from config appRoutes route.name === "MCT")
      ...
      {(typeof description === 'string' && !description.toLowerCase().includes("customerror")) && (
        <>
          ...
          <p className={classesStyle.para}>
            ...
            <Link
              href={process.env.REACT_APP_ReportMCTIncident}
              underline
