...
import { IAppContext, IAppTheme, IExtensionContext } from "@msx/platform-services";
...
class ExtensionContext implements IExtensionContext {
  public readonly key: string = process.env.REACT_APP_EXTENSION_KEY ;
  public readonly name: string = process.env.REACT_APP_EXTENSION_NAME ;

  public appContext: IAppContext | null = null;

  constructor() {
      this.key = process.env.REACT_APP_EXTENSION_KEY;
      this.name = process.env.REACT_APP_EXTENSION_NAME ;
  }
}
...
export const getErrorMessage = (response?: any): string => {
  ...
  switch (response?.status) {
    ...
    case 422:  
      ...
      break;
    case 403:
      if (responseData?.toLowerCase().includes("customerror".toLowerCase())) {
