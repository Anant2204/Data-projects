import { Dropdown, Label, Stack, TextField, classNamesFunction } from "@fluentui/react";
import React, { useContext, useEffect, useState } from "react";
import * as constants from "../HelperFiles/rrFormConstants";
import { SalesUnitDropdown, StaticDropdown, TPIDDynamicDropdown } from "../../WebFormUtils/commonComponents";
import { ApplicationContext } from "@msx/platform-services";
import { getStyles } from "../RevenueRequests.styles";
export interface RevenueMisalignedToMyManagedAccountProp {
  config?: any;
...
const RevenueMisalignedToMyManagedAccount: React.FC<
  RevenueMisalignedToMyManagedAccountProp
> = ({
  config,
  selectedRequestType,
  formData,
      ...
      >
        ...
        <Label>
          <span
            ...
            {
              config?.[constants.SERVICE_NAME_REQ_TYPE(selectedRequestType)]
                ?.typeOfRequest[selectedRequestType]?.["What is the impacted Revenue amount in $?"]?.fieldName
            }
            </span>
          </Label>
          <TextField
      ...
      >
        ...
        {salesUnitSelected && (
          ...
          <span
            ...
            {
              config?.[constants.SERVICE_NAME_REQ_TYPE(selectedRequestType)]
                ?.typeOfRequest[selectedRequestType]?.["Impacted TPID"]?.fieldName
            }
            </span>
          </Label>
            <TPIDDynamicDropdown
        ...
        <Label>
          <span
            ...
            {
              config?.[constants.SERVICE_NAME_REQ_TYPE(selectedRequestType)]
                ?.typeOfRequest[selectedRequestType]?.["Related ID Type"]?.fieldName
            }
            </span>
          </Label>
        <StaticDropdown
        ...
        <Stack styles={{ root: { width: "33%" } }}>
          {relatedIdTypeSelected && (
            ...
            <span
              ...
              {
                config?.[constants.SERVICE_NAME_REQ_TYPE(selectedRequestType)]
                  ?.typeOfRequest[selectedRequestType]?.["Related ID Value"]?.fieldName
              }
              </span>
            </Label>
            <TextField
...
export default RevenueMisalignedToMyManagedAccount;
